...
화면별 상세 스펙은 [`docs/screens.md`](docs/screens.md) 참고.

## 기술 스택
- Kotlin + Jetpack Compose, 최소 API 26
...
val FABBrown         = Color(0xFF9A7B5E) // 매칭 + FAB
...
| headlineMedium | 20 | ExtraBold | 32 | 서브페이지 TopBar 제목(알림·설정·반려동물추가 등) |
...
- 바: White bg, `RoundedCornerShape(50.dp)`, elevation=8dp, 좌우 margin=16dp, 바닥 margin=12dp
- 선택탭: 검정(#1A1A1A) pill + 아이콘(24dp) + 텍스트(14sp Medium) White, pill padding h=16dp v=10dp
- 미선택탭: 아이콘(24dp)만, Brown400(#C4A882), 텍스트 없음

### TopBar (뒤로가기 화면)
- 뒤로가기: 흰 카드(40×40dp, radius=12dp, elevation=2dp) + KeyboardArrowLeft
